...
const TableTenant = () => {
  ...
  const filteredtenantCompData = tenantData.companies?.filter(
    (booking: any) => {
  ...
  const filteredtenantIndData = tenantData.independents?.filter(
    (booking: any) => {
      const bookingStart = new Date(booking.start_time);
      const bookingEnd = new Date(booking.end_time);
      const start = new Date(startDate);
  ...
  return (
    <div className="rounded-sm border border-stroke bg-white px-5 pb-2.5 pt-6 shadow-default dark:border-strokedark dark:bg-boxdark sm:px-7.5 xl:pb-1">
      ...
      btn-primary m-4 mb-4 inline-flex items-center justify-center rounded-md border border-slate-950 px-10 py-4 text-center font-medium text-black hover:bg-black hover:text-white lg:px-8 xl:px-10"
            ...
            <button
              onClickCapture={openIndepententTable}
              className="btn btn-primary btn 
      btn-primary m-4 mb-4 inline-flex items-center justify-center rounded-md border border-slate-950 px-10 py-4 text-center font-medium text-black hover:bg-black hover:text-white lg:px-8 xl:px-10"
            >
              Independent
      ...
      <div className="max-w-full overflow-x-auto">
        {isCompanyOpen ? (
          <table className="w-full table-auto">
            ...
            <tbody>
              {filteredtenantCompData?.map((comp: any, index: any) => (
                <tr key={comp.id}>
        ...
        {isIndOpen ? (
          <table className="w-full table-auto">
            ...
            <tbody>
              {filteredtenantIndData?.map((ind: any, index: any) => (
                <tr key={ind.id}>
                  <td className="border-b border-[#eee] px-4 py-5 pl-9 dark:border-strokedark xl:pl-11">
                    <h5 className="font-medium text-black dark:text-white">
                      {index + 1}
